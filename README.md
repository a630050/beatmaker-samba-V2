...
### 進階編輯
1. **選擇模式**：點擊「進階編輯」按鈕進入進階編輯模式
2. **框選區域**：拖拽滑鼠選取多個格子(可以跨音軌選取)
3. **複製**：選取後點擊「複製」
4. **貼上**：在目標位置點擊「貼上」
5. **儲存樣式**：導出框選區域之音格樣式
6. **載入樣式**：導入之前存好的音格樣式
...
- **存檔**：將編曲儲存為 JSON 檔案
- **載入**：載入之前儲存的編曲
- **匯出MP3**：將編曲轉換為音頻檔案 (支援設定當前播放長度之循環次數為mp3聲音檔)
...
### Samba Beatmaker 快速鍵功能清單

您可以將此清單分享給團隊成員，方便大家記憶與操作。

#### **一、 播放控制 (Playback Control)**

| 功能 | 按鍵 | 說明 |
| :--- | :--- | :--- |
| **播放 / 暫停** | `Space` (空白鍵) 或 `P` | 開始播放或在目前位置暫停。 |
| **停止並回到開頭** | `S` | 無論在哪個位置，立即停止播放並將播放軸歸零。 |
| **從頭播放** | `B` | 立即從第一拍開始播放。如果正在播放，會先停止再從頭播放。 |
| **增加 / 減少 BPM** | `↑` / `↓` (方向鍵) | 微調曲速 (BPM)。 |
| **增加 / 減少 總音量** | `→` / `←` (方向鍵) | 微調主音量。 |

#### **二、 模式與編輯 (Editing & Modes)**

| 功能 | 按鍵 | 說明 |
| :--- | :--- | :--- |
| **切換 [編輯] 模式** | `E` | 進入或退出用於複製/貼上的「編輯」模式。 |
| **切換 [標記] 模式** | `M` | 進入或退出用於標記 R/L/強弱音的「標記」模式。 |
| **複製選取範圍** | `Ctrl` + `C` | 在 [編輯] 模式下，複製已選取的格子範圍。 |
| **貼上樣式** | `Ctrl` + `V` | 在 [編輯] 模式下，將複製的樣式貼上到指定位置。 |
| **上一步** | `Ctrl` + `Z` | 復原上一個操作。 |
| **下一步** | `Ctrl` + `Y` | 重做上一個復原的操作。 |
| **退出模式 / 取消** | `Esc` | 關閉 [編輯] 或 [標記] 模式，或取消點擊播放軸後的高亮狀態。 |

#### **三、 標記工具快捷鍵 (Marker Tools)**

*注意：以下快捷鍵僅在按下 `M` 進入**[標記] 模式**後生效。*
...
| **選擇橡皮擦** | `X` | 切換到橡皮擦工具，用於清除格子上的所有標記。 |

#### **四、 檔案操作 (File Operations)**
